...
function App() {
  ...
  if (serviceRoute) content = <ServiceLandingPage service={serviceRoute} onStartProject={startProject} onNavigateServices={() => navigate('/services')} />;
    ...
    <section className="py-20 bg-[#0B0C10]"><div className="max-w-6xl mx-auto px-4 grid md:grid-cols-3 gap-5">
      {[['General enquiries','info@korvenzatech.com',Mail],['Project & sales','sales@korvenzatech.com',MessageSquare],['Location','Pakistan • Serving clients globally',MapPin]].map(([a,b,I]:any)=><div key={a} className="premium-card p-7"><I className="w-5 h-5 text-emerald-300"/><h3 className="font-bold text-lg mt-5">{a}</h3><p className="text-slate-400 text-sm mt-2">{b}</p></div>)}
    ...
    <section className="py-24 bg-[#0B0C10] border-t border-white/7"><div className="max-w-7xl mx-auto px-4">
      <div className="max-w-3xl"><div className="text-emerald-300 text-xs uppercase tracking-[.2em] font-bold">Core capabilities</div><h2 className="mt-4 text-3xl sm:text-5xl font-extrabold tracking-tight">One technology partner, from idea to launch.</h2><p className="mt-5 text-slate-400 text-lg leading-8">We keep the homepage focused. Explore dedicated pages when you want the technical detail.</p></div>
    ...
    <section className="py-24 bg-[#0a1322]"><div className="max-w-7xl mx-auto px-4"><div className="flex items-end justify-between gap-6 mb-10"><div><div className="text-emerald-300 text-xs uppercase tracking-[.2em] font-bold">Live products</div><h2 className="text-3xl sm:text-5xl font-extrabold mt-4">Proof, not promises.</h2></div><button onClick={openWork} className="hidden sm:flex text-sm font-bold text-emerald-300 items-center gap-2">All case studies <ArrowRight className="w-4 h-4"/></button></div>
      <div className="grid md:grid-cols-3 gap-5">{CASE_STUDIES.map(c=><button key={c.id} onClick={()=>setStudy(c)} className="premium-card overflow-hidden text-left group"><img src={c.imageSrc} className="w-full h-48 object-cover opacity-75 group-hover:opacity-100 group-hover:scale-[1.02] transition-all duration-500"/><div className="p-6"><div className="text-[10px] text-emerald-300 uppercase tracking-wider">{c.category}</div><h3 className="text-xl font-bold mt-3">{c.name}</h3><p className="text-sm text-slate-400 mt-2 line-clamp-2">{c.tagline}</p><div className="mt-5 text-xs font-bold text-emerald-300">View case study →</div></div></button>)}</div>
    </div></section>
    <WhyKorvenzaTech/>
