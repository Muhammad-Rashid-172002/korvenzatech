export function buildAssistantInstruction(memoryText: string) {
  ...
  const whatsappUrl = process.env.PUBLIC_WHATSAPP_URL?.trim() || '';

  const verifiedContact = [
    siteUrl ? `- Official website: ${siteUrl}` : '',
    contactEmail ? `- General enquiries: ${contactEmail}` : '',
    salesEmail ? `- Project & sales: ${salesEmail}` : '',
    contactPhone ? `- Official business phone / WhatsApp: ${contactPhone}` : '',
