...
export async function handleContact(
  ...
  const inquiryType = String(body?.inquiryType || body?.serviceNeeded || 'General Inquiry').trim();

  if (!fullName || !email) {
    return { status: 400, body: { error: 'Name and email are required fields.' } };
  }
  if (fullName.length > 120 || email.length > 180 || (body?.ideaDescription && String(body.ideaDescription).length > 8000)) {
  ...
  // Honeypot protection
  if (body?.website) {
    return { status: 200, body: { success: true, message: 'Thank you. Your inquiry has been received.' } };
  }

  const referenceId = `KZ-${Date.now().toString(36).toUpperCase()}-${Math.random().toString(36).slice(2, 6).toUpperCase()}`;
  const normalized = inquiryType.toLowerCase();
  const isSales = /sales|project|software|saas|mobile|website|api|cloud|ui\/ux|digital|ai solution|not sure|budget/.test(normalized);
  const destinationEmail = isSales ? 'sales@korvenzatech.com' : 'info@korvenzatech.com';
  ...
  const payload = {
    ...
    ideaDescription: String(body?.ideaDescription || body?.message || '').slice(0, 8000),
    preferredContact: String(body?.preferredContact || 'Email').slice(0, 80),
    whatsapp: '+923701691701',
  };

  // Production delivery uses a Google Apps Script / trusted automation webhook.
  // The included scripts/google-workspace-contact-webhook.gs sends directly into
  // info@korvenzatech.com or sales@korvenzatech.com based on destinationEmail.
  const webhook = process.env.CONTACT_WEBHOOK_URL?.trim();
  if (!webhook) {
    console.warn('Contact delivery is not configured. Add CONTACT_WEBHOOK_URL in Vercel.', { referenceId, destinationEmail });
    return { status: 503, body: { error: 'Contact delivery is not configured yet. Please email info@korvenzatech.com or sales@korvenzatech.com directly.' } };
  }
  ...
  try {
    ...
    return { status: 502, body: { error: 'We could not deliver your inquiry right now. Please try again shortly.' } };
  }
  ...
  return {
    ...
    body: {
      ...
      message: `Thank you. Your inquiry has been routed to ${destinationEmail}.`,
      referenceId,
    },
  };
}
